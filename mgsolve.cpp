...
inline void resdualNorm(const Grid* xgrd, const Grid * fgrd, double* norm)
{
   ...
   // double	beta = 1.0;
	...
	double sum = 0.0;
	//std::cout << "***************:: Residual= ";
#pragma omp parallel reduction(+: sum) firstprivate(r) //firstprivate(dimX,dimY,midX,midY,hx,hy,r)
	{
#pragma omp for
...
inline void errorNorm(const Grid* xgrd, const Grid * sgrd, double* norm)
{
	...
	//size_t j;
#pragma omp parallel reduction(+: sum) firstprivate(r)//firstprivate(dimY, dimX,r)
	{
#pragma omp for
